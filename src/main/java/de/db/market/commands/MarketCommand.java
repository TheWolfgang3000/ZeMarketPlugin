...
public class MarketCommand implements CommandExecutor {
        ...
        switch (shopAction) {
            ...
            case "delete":
                ...
                if (shopToDelete == null) {
                    player.sendMessage("§c[MarketSystem] §fDu stehst in keinem Shop-Plot, der geloescht werden kann.");
                    return;
                }
                shopManager.removeShop(shopToDelete);
                player.sendMessage("§a[MarketSystem] §fDer Shop-Plot wurde erfolgreich geloescht.");
                break;
    ...
    private void handleRegionCommand(Player player, String[] args) {
        ...
        switch (regionAction) {
            ...
            case "pos2":
                ...
                break;
            case "create":
                if (args.length < 3) { player.sendMessage("§c[MarketSystem] §fBitte gib einen Namen an: /market region create <name>"); return; }
                if (!pos1Selections.containsKey(playerName) || !pos2Selections.containsKey(playerName)) { player.sendMessage("§c[MarketSystem] §fDu musst zuerst beide Positionen setzen!"); return; }
                String regionName = args[2];
                if (regionManager.regionExists(regionName)) { player.sendMessage("§c[MarketSystem] §fEine Region mit diesem Namen existiert bereits!"); return; }
            ...
            case "delete":
                if (args.length < 3) { player.sendMessage("§c[MarketSystem] §fBitte gib einen Namen an: /market region delete <name>"); return; }
                String regionToDelete = args[2];
                if (!regionManager.regionExists(regionToDelete)) { player.sendMessage("§c[MarketSystem] §fEine Region mit diesem Namen wurde nicht gefunden."); return; }

                // Zuerst alle Shops in der Region löschen
                shopManager.removeShopsInRegion(regionToDelete);

                // Dann die Region selbst löschen
                regionManager.removeRegion(regionToDelete);

                player.sendMessage("§a[MarketSystem] §fRegion '" + regionToDelete + "' und alle darin enthaltenen Shops wurden geloescht.");
                break;
            default:
                player.sendMessage("§c[MarketSystem] §fUnbekannter Region-Befehl.");
                break;
        }
    }
    ...
    private void sendHelpMessage(Player player) {
        ...
        if (player.isOp()) {
            ...
            player.sendMessage("§6/market shop delete §f- Loescht den Shop, in dem du stehst.");
        }
    }
